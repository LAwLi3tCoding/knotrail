...
| PlanRevision | 完整节点、依赖、检查引用、摘要与 digest | 提交校验后追加 |
...
| CheckReceipt | 固定条件 ID、pass/fail/unknown、文件摘要和输出 | Main 的检查器生成 |
...
提交后关闭该 Run 的后续工具准入，等待 Worker 结束，再进入 `ready`。`reviewBeforeExecute` 停在这里等待用户开始；`autoWithinGrant` 在既有任务权限内自动继续。关闭右侧面板不会改变这段协议。
...
检查命令在创建任务时保存为 argv 数组，不通过 shell 字符串拼接。模型不能替换用户的检查定义。保护路径不能被工具或检查命令改写；如果检查本身改变被摘要覆盖的工作树内容，本次检查判失败。
...
恢复时比较当前摘要和最近有效证据；发生变化则使节点证据失效。节点产物保存的是运行结束时的累计工作树 diff，并非可独立应用和回滚的节点 patch。
