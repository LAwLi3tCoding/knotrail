...
重试会使指定节点、已有尝试及其依赖后继失效。当前提示包含全任务历史，尚不能用文件未变证明研究结论可复用；未运行且不受影响的排队节点可保留。重试从现在的文件状态开始，历史效果不会被隐式回滚；执行提示包含之前的结果和已回答决定。工作树没有自动 reset、stash、clean，也不会自动 push、merge 或发布。
