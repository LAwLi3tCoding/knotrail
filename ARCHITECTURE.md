...
检查命令在创建任务时保存为 argv 数组，不通过 shell 字符串拼接。模型不能替换用户的检查定义。保护路径不能被工具或检查命令改写。同一检查批次绑定唯一的 TaskRevision、Plan ID、检查定义摘要和工作区摘要；若任务消费过等待来源，还绑定 observationDigest 并重新读取该来源。检查前后、检查之间及最终提交时都要核对这些绑定。不同文件或观察版本的 pass 不能合并验收，最终 acceptedDigest 只能取已验证的工作区摘要。这是当前单个登记来源的查证，还不是原设计中的完整外部输入向量。
