...
1. **补足 T12/T18 的真实故障证据。** 工作区批次、存储修复及本轮观察修复已有独立复核和打包态验证；最新 check 84 项中 83 通过、1 项预期跳过。验收修订新增真实 SQLite 事务最后写失败的回滚验证；后续仍需构造真实 I/O、磁盘满和强杀窗口，不能将触发器、Store.put 注入或当前通过结果当作这些实验已经完成。
