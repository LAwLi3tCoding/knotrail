...
2026-09-07 新增要求：默认快速开启会话并在同一工作区持续对话，无需每次填写大量配置。快速入口、task.message、同会话历史和逐消息预算已实现；Core 50/50、真实 pi 与 Codex 协议 17/17、全套 UI 46/46 与开发态两轮对话桌面链路已通过。审查发现的侧栏回退、偏好丢稿和同事件序列旧快照覆盖暂停状态，三份原始反例原样 3/3 通过；实际桌面发现的滚动跟随问题也已修复，最终打包态原视口断言通过，九个构建文件摘要一致，最终独立复核为 pass。输入/产物绑定已进入下一阶段实现，当前验证另列；完整产品目标继续保持未完成。

2026-09-07 输入/产物阶段：typed 声明、当前生产者绑定、完整文件与 summary 快照、分段 read_input、来源/保存文本双摘要、Run 自有输入展示、结构化实际读取及研究只读已写入。重试保守撤销既有尝试，尚不等于语义研究复用。该阶段已通过 103 项 Core 回归、真实 pi 协议、受影响 UI、最终打包桌面链与独立只读复核。决定/等待收尾状态、后继准入及验收来源一致性反例已修复，后面的旧阶段表仍保留其当时证据；不能据此把完整产品标为完成。

2026-09-07 文件意图阶段：确定性文件工具新增先持久化再 ACK 的写入协议，显式恢复或检查可核对当前后置状态；匹配后保留 unknown 历史并重新规划。当前完整 check 为 167 项，166 通过、1 条反向沙箱能力用例预期跳过；包含 Core 109/109、真实宿主三个强杀窗口与完整执行器回归。受影响双语 UI 3/3、最新打包桌面链及包内摘要核对通过。独立只读复核为 pass，另有三项通知失败/ACK 前取消探针通过。该阶段已完成本地交付验证。以下阶段表保留先前审查时的证据，文件 witness 的缺失描述由本段更新；完整后代收尾、磁盘故障矩阵及全目标仍未完成。

## 1. 审查口径

2026-09-07 VM 实验更新：固定 Apple container 1.3.1 已完成真实启动及六组可重复原语探针，复现只读叶节点的祖先改名绕过和 CLI 死亡后继续写入；祖先挂载与三种运行时收尾路径的具体观察见 [VM 隔离实验](VM-ISOLATION.md)。这改变了下一步接入依据，但没有替换当前执行器，不能把 T05/T06/T07 或完整目标改为完成。
