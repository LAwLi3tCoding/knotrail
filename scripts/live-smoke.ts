...
const authSource=process.env.KNOTRAIL_AUTH_SOURCE??'api-key';if(!['api-key','codex-login'].includes(authSource))throw new Error('KNOTRAIL_AUTH_SOURCE must be api-key or codex-login');
const baseUrl=authSource==='codex-login'?CODEX_BASE_URL:process.env.KNOTRAIL_BASE_URL??'https://api.openai.com/v1';
const key=authSource==='api-key'?process.env.KNOTRAIL_API_KEY:undefined;
...
const app=createAppService({dataDir,secretStore:{get:()=>key?JSON.stringify({key,baseUrl}):undefined,set:()=>{}},lockFd:owner.fd,capabilities:capability,notify:()=>{}});
try {
 await app.command({type:'settings.save',patch:{model:{authSource:authSource as 'api-key'|'codex-login',baseUrl,modelId,thinking:authSource==='codex-login'?'high':'off'},responseLanguage:'en'}});
 ...
 const deadline=Date.now()+360000;let lastStatus='';
 while(Date.now()<deadline){const s=await app.command({type:'task.snapshot',taskId:task.task.id}) as TaskSnapshot;if(s.task.status!==lastStatus){lastStatus=s.task.status;console.log(JSON.stringify({event:'task.status',status:lastStatus,turns:s.task.turnCount,nodes:s.plan?.nodes.length??0}));}if(s.task.status==='completed'){if(readFileSync(join(s.task.workdir,'target.txt'),'utf8')!=='after\n')throw new Error('Final output mismatch');console.log(JSON.stringify({status:'passed',authSource,modelId,plans:s.plans.length,runs:s.runs.length,checks:s.checks.length,usage:s.runs.map(r=>r.usage)}));break;}if(['blocked','waiting_user','expired','cancelled'].includes(s.task.status))throw new Error(`Live task stopped: ${s.task.status}: ${s.task.error??'Requires a user decision'}`);await new Promise(resolve=>setTimeout(resolve,500));if(Date.now()>=deadline)throw new Error('Live smoke deadline exceeded');}
