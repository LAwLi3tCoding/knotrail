...
export class AppService {
 ...
 private async dispatch(c:AppCommand):Promise<CommandResult> {
  ...
  if(c.type==='task.message') {
   ...
   s.task.revision++;s.task.objective=message;s.task.turnBudgetStart=s.task.turnCount;s.task.status='planning';s.task.activePlanId=undefined;s.task.acceptedDigest=undefined;s.task.acceptedSourcesDigest=undefined;s.task.error=undefined;s.task.wait=undefined;s.task.health=undefined;s.task.consumedObservations=undefined;s.task.nextCheckAt=undefined;s.task.updatedAt=now();
  ...
  if(c.type==='task.applyImpact') {
   ...
   const fresh=this.preview(p.taskId,p.objective,p.nodeId,p.checks);if(fresh.expectedPlanId!==p.expectedPlanId||fresh.workspaceDigest!==p.workspaceDigest||fresh.inputSourcesDigest!==p.inputSourcesDigest)throw new Error('Plan or workspace changed; generate a new impact preview');
 ...
 private preview(taskId:string,objective?:string,nodeId?:string,checks?:CheckSpec[]):ImpactPreview {
  ...
  // Current prompts include task-wide history. Unchanged file hashes alone do not prove a prior analysis is reusable.
  const affected=new Set(revised?all:[nodeId!,...s.nodes.filter(n=>n.attempt>0).map(n=>n.nodeId)]);let changed=true;while(changed){changed=false;for(const n of s.plan?.nodes??[])if(n.dependsOn.some(d=>affected.has(d))&&!affected.has(n.id)){affected.add(n.id);changed=true;}}
  const inputSourcesDigest=this.knownSources(s).digest;
  return {id:id(),taskId,expectedRevision:s.task.revision,expectedPlanId:s.task.activePlanId,workspaceDigest:workspaceDigest(s.task.workdir),inputSourcesDigest,objective,checks,nodeId,affected:[...affected],retained:all.filter(n=>!affected.has(n)),reason:checks!==undefined?'Acceptance checks changed: previous results remain historical; the revised task must be planned and verified again.':objective!==undefined?'Objective changed: all previous nodes lose completion authority; history and files remain available.':'Retry invalidates prior attempts and dependent nodes because prompts share task history and implicit inputs may be unknown. Existing files and artifacts remain historical; no effects are rolled back automatically.'};
 }
 ...
 private continuationCurrent(s:TaskSnapshot,currentDigest=workspaceDigest(s.task.workdir)):boolean {
  const index=s.runs.findLastIndex(r=>s.nodes.some(n=>n.runId===r.id&&(finishedNodes.has(n.status)||(r.suspendedState&&n.status==='stale'&&!n.reason))));if(index<0)return true;
 ...
 private reconcile(taskId:string):void {
  const s=this.current(taskId),current=workspaceDigest(s.task.workdir);this.update(taskId,v=>{
   if(v.plan&&!v.nodes.some(n=>n.attempt>0)&&!this.planningCurrent(v,current)){v.plan=undefined;v.draft=undefined;v.task.activePlanId=undefined;v.nodes=[];v.task.status='planning';this.store.event(v,'plan.stale','Planning input changed before execution; a fresh plan is required');return;}
  ...
  try {
   ...
   if(terminal.kind==='update_plan'){
    const candidate=terminal.draft;this.update(taskId,s=>{this.assertObservation(s);
     ...
     s.plans.push(plan);s.plan=plan;s.task.activePlanId=plan.id;s.nodes=plan.nodes.map(n=>({nodeId:n.id,status:'queued',attempt:0}));
 ...
 private batchMatches(s:TaskSnapshot,batch:CheckBatch,currentDigest=workspaceDigest(s.task.workdir)):boolean {
  ...
  return sources.complete&&sources.digest===batch.inputSourcesDigest&&s.task.revision===batch.taskRevision&&s.task.activePlanId===batch.planId&&digest(JSON.stringify(s.task.checks))===batch.checksDigest&&currentDigest===batch.inputDigest&&observation===batch.observationDigest&&this.observationCurrent(s);
 }
