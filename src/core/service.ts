...
export class AppService {
 ...
 private queue=new Set<string>();private active?:{taskId:string;controller:AbortController;done:Promise<void>};private closing=false;private serial=Promise.resolve();private timer:NodeJS.Timeout;
 ...
 private async dispatch(c:AppCommand):Promise<CommandResult> {
  ...
  if(c.type==='task.resume') {
   ...
   await this.stop(c.taskId);this.reconcile(c.taskId);this.update(c.taskId,v=>{v.task.status=v.plan?'executing':'planning';v.task.error=undefined;v.task.nextCheckAt=undefined;this.store.event(v,'resumed','Task resumed after workspace reconciliation');});this.enqueue(c.taskId);return this.current(c.taskId);
  }
  if(c.type==='task.previewRevision'||c.type==='task.previewRetry') {this.current(c.taskId,c.expectedRevision);await this.stop(c.taskId);this.update(c.taskId,s=>{if(!stopped.has(s.task.status))s.task.status='paused';});const p=this.preview(c.taskId,c.type==='task.previewRevision'?c.objective:undefined,c.type==='task.previewRetry'?c.nodeId:undefined);this.store.set('preview:'+p.id,p);return p;}
  ...
  if(c.type==='decision.answer') {
   const fingerprint=digest(JSON.stringify(c));const old=this.store.request<string>(c.requestId,fingerprint);if(old)return this.current(old);
   const s=this.current(c.taskId,c.expectedRevision);if(s.task.status!=='waiting_user')throw new Error('Task is not waiting for a decision');if(this.isExpired(s)){this.expire(c.taskId);throw new Error('Task deadline reached');}const decision=s.decisions.find(d=>d.id===c.decisionId&&d.taskRevision===s.task.revision&&!d.answer);if(!decision)throw new Error('Decision is missing or already answered');if(!decision.options.includes(c.answer))throw new Error('Select one of the offered answers');
   if(decision.question.startsWith('[acceptance]')) {
 ...
 private reconcile(taskId:string):void {const s=this.current(taskId),current=workspaceDigest(s.task.workdir);this.update(taskId,v=>{const trusted=v.task.acceptedDigest??v.nodes.findLast(n=>n.status==='verified')?.outputDigest;if(trusted&&trusted!==current){for(const n of v.nodes){n.status='stale';n.reason='Workspace changed since verification';}this.store.event(v,'evidence.stale','Workspace changed; node evidence invalidated');}for(const n of v.nodes)if(['running','unknown','failed'].includes(n.status))n.status='stale';});}
 ...
 private pump():void {
  ...
  const done=Promise.resolve().then(()=>this.drive(taskId,controller.signal)).catch(error=>{if(!this.closing)this.update(taskId,s=>{if(!stopped.has(s.task.status)&&s.task.status!=='paused'){s.task.status=this.isExpired(s)?'expired':'blocked';s.task.error=this.clean(error instanceof Error?error.message:String(error));this.store.event(s,'error',s.task.error);}});}).finally(()=>{this.active=undefined;this.pump();});
 ...
 private async drive(taskId:string,signal:AbortSignal):Promise<void> {
  let s=this.current(taskId);if(!['planning','executing','verifying'].includes(s.task.status))return;
  ...
  while(!signal.aborted){s=this.current(taskId);if(s.task.status!=='executing')return;
 ...
 private async run(taskId:string,nodeId:string|undefined,signal:AbortSignal):Promise<void> {
  const start=this.current(taskId);const parentSignal=signal;const timeoutMs=Math.max(1,Math.min(start.task.maxRunMs,start.task.expiresAt?Date.parse(start.task.expiresAt)-Date.now():Infinity));signal=AbortSignal.any([signal,AbortSignal.timeout(timeoutMs)]);const node=start.plan?.nodes.find(n=>n.id===nodeId),inputDigest=workspaceDigest(start.task.workdir),before=workspaceDiff(start.task.workdir),runId=id();
  ...
  let terminal:RunControl|undefined;let admission=true;let observedTurns=0;
  const tool=async(call:ToolCall):Promise<ToolResult>=>{
   ...
   const receipt=id();this.update(taskId,s=>{s.actions.push({id:receipt,taskId,runId,nodeId,toolCallId:call.toolCallId,name:call.name,argsDigest:digest(JSON.stringify(call.args)),status:'pending',startedAt:now()});this.store.event(s,'tool.started',call.name,{runId,nodeId,data:{toolCallId:call.toolCallId,args:JSON.parse(this.clean(JSON.stringify(call.args)))}});});
   try {const result=await this.executor.execute(call,{workdir:start.task.workdir,dataDir:this.options.dataDir,allowNetwork:this.settings().allowNetwork,lockFd:this.options.lockFd,timeoutMs:Math.min(start.task.maxRunMs,120000),protectedPaths:[...new Set(start.task.checks.flatMap(c=>c.protectedPaths))]},signal);
    const output=this.clean(result.text).slice(0,64000);this.update(taskId,s=>{const r=s.actions.find(a=>a.id===receipt)!;r.status=signal.aborted?'unknown':result.isError?'failed':'succeeded';r.output=output;r.endedAt=now();this.store.event(s,'tool.finished',output,{runId,nodeId,data:{name:call.name,isError:result.isError}});});return {...result,text:output};
   }catch(e){this.update(taskId,s=>{const r=s.actions.find(a=>a.id===receipt)!;r.status='failed';r.output=this.clean(String(e));r.endedAt=now();});return {text:this.clean(String(e)),isError:true};}
  };
  try {
   const result=await this.runner.run({runId,purpose:run.purpose,workdir:start.task.workdir,sessionDir:join(this.options.dataDir,'sessions',runId),model:this.model(),objective:start.task.objective,checks:start.task.checks,plan:start.plan,node,context:JSON.stringify({previousRuns:start.runs.slice(-8).map(r=>({node:r.nodeId,status:r.status,summary:r.summary})),decisions:start.decisions.filter(d=>d.answer),workspaceDigest:inputDigest}),maxTurns:start.task.maxTurns-start.task.turnCount,timeoutMs:start.task.maxRunMs,responseLanguage:this.settings().responseLanguage}, {
    onEvent:(kind,text,data)=>{if(admission&&!signal.aborted){if(kind==='turn.started'){observedTurns++;this.update(taskId,s=>{s.task.turnCount++;});}this.event(taskId,kind,text,run,data);}},onTool:tool,
    onControl:async(control)=>{
     if(signal.aborted||!admission)return {text:'Run admission is closed',isError:true};
     if(control.kind==='update_plan'){
      if(node)return {text:'Only the planning run can change the plan',isError:true};const draft=validatePlan(control.draft,start.task.checks,control.submit);const s=this.current(taskId);if(s.draft&&draft.sequence<=s.draft.sequence)return {text:'Plan draft sequence is stale',isError:true};
      this.update(taskId,v=>{v.draft=draft;this.store.event(v,'plan.draft',draft.summary,{runId,data:draft});if(control.submit){const plan:PlanRevision={...draft,id:id(),taskRevision:v.task.revision,revision:v.plans.length+1,createdAt:now(),digest:digest(JSON.stringify(draft))};v.plans.push(plan);v.plan=plan;v.task.activePlanId=plan.id;v.nodes=plan.nodes.map(n=>({nodeId:n.id,status:'queued',attempt:0}));this.store.event(v,'plan.committed','Validated plan committed',{runId,data:{planId:plan.id}});}});
      if(control.submit){terminal=control;admission=false;}return {text:control.submit?'Plan committed':'Draft saved'};
     }
   ...
   admission=false;this.update(taskId,s=>{const r=s.runs.find(r=>r.id===runId)!;r.status=result.aborted||signal.aborted?'aborted':'succeeded';r.endedAt=now();r.summary=this.clean(result.summary);r.usage=result.usage;r.sessionPath=result.sessionPath;if(!observedTurns)s.task.turnCount+=Math.max(1,result.turns);});
 ...
 private async check(taskId:string,runId:string,nodeId:string|undefined,conditionIds:string[],signal:AbortSignal):Promise<boolean> {
  const s=this.current(taskId);let ok=true;
  for(const condition of s.task.checks.filter(c=>conditionIds.includes(c.id))) {
   if(signal.aborted)return false;const before=workspaceDigest(s.task.workdir),receipt=id();
   this.update(taskId,v=>v.actions.push({id:receipt,taskId,runId,nodeId,toolCallId:'check:'+condition.id+':'+id(),name:'required_check',argsDigest:digest(JSON.stringify(condition.command)),status:'pending',startedAt:now()}));
   let result:ToolResult;try {result=await this.executor.execute({toolCallId:receipt,name:'run_command',args:{argv:condition.command}},{workdir:s.task.workdir,dataDir:this.options.dataDir,allowNetwork:this.settings().allowNetwork,lockFd:this.options.lockFd,timeoutMs:Math.min(s.task.maxRunMs,120000),protectedPaths:[...new Set(s.task.checks.flatMap(c=>c.protectedPaths))]},signal);}catch(e){result={text:String(e),isError:true};}
   const after=workspaceDigest(s.task.workdir),status=signal.aborted?'unknown':result.isError||before!==after?'fail':'pass';if(status!=='pass')ok=false;
   const output=this.clean(result.text)+(before!==after?'\nCheck modified workspace files; verification is invalid.':'');
   this.update(taskId,v=>{const action=v.actions.find(a=>a.id===receipt)!;action.status=status==='pass'?'succeeded':status==='fail'?'failed':'unknown';action.output=output.slice(0,64000);action.endedAt=now();v.checks.push({id:id(),taskId,runId,nodeId,conditionId:condition.id,result:status,inputDigest:before,output:output.slice(0,64000),checkedAt:now()});this.store.event(v,'check.finished',condition.label+': '+status,{runId,nodeId,data:{conditionId:condition.id,result:status}});});
  }return ok;
 }
 private async finalize(taskId:string,signal:AbortSignal):Promise<void> {
  const s=this.current(taskId);if(this.isExpired(s)){this.expire(taskId);return;}this.update(taskId,v=>{v.task.status='verifying';});const runId=s.runs.at(-1)?.id??id();
  const checkSignal=s.task.expiresAt?AbortSignal.any([signal,AbortSignal.timeout(Math.max(1,Date.parse(s.task.expiresAt)-Date.now()))]):signal;const ok=await this.check(taskId,runId,undefined,s.task.checks.map(c=>c.id),checkSignal);if(this.isExpired(this.current(taskId))){this.expire(taskId);return;}if(signal.aborted)return;
  this.update(taskId,v=>{if(!ok){v.task.status='blocked';v.task.error='Final acceptance checks failed';}else{v.task.acceptedDigest=workspaceDigest(v.task.workdir);if(this.isExpired(v)){v.task.status='expired';this.store.event(v,'expired','Task deadline reached during final verification');return;}if(v.task.checks.length){v.task.status=v.task.mode==='maintain'?'healthy':'completed';this.scheduleNext(v);}else{v.task.status='waiting_user';v.decisions.push({id:id(),taskId,taskRevision:v.task.revision,planId:v.task.activePlanId,question:'[acceptance] No automated acceptance commands were supplied. Review the artifacts and accept this exact workspace result.',options:['accept','reject'],createdAt:now()});}}this.store.event(v,'verification.finished',v.task.status,{data:{digest:v.task.acceptedDigest}});});
 }
