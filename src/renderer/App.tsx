...
import type { AppCommand, AppSettings, Artifact, Bootstrap, CheckSpec, Decision, FileSource, ImpactPreview, InputBinding, InputRef, Locale, OutputSpec, PlanNode, PlanRevision, Task, TaskEvent, TaskPreferences, TaskSnapshot } from '../shared/contracts';
...
function IODeclaration({ value, t }: { value: InputRef | OutputSpec | string; t: T }) {
  if (typeof value === 'string') return <>{value}</>;
  return <span className="io-declaration">{'id' in value && <code>{value.id}</code>}{value.kind === 'artifact' ? <>{t('Step artifact')} · <code>{value.nodeId} / {value.outputId}</code></> : value.kind === 'file' ? <>{t('File')} · <code>{value.path}</code><small>{t(value.expect === 'present' ? 'Must exist' : 'Must be absent')}</small></> : t('Text summary')}</span>;
}
function IODeclarations({ values, t }: { values: (InputRef | OutputSpec | string)[]; t: T }) {
  return values.length ? <ul className="io-declarations">{values.map((value, index) => <li key={index}><IODeclaration value={value} t={t} /></li>)}</ul> : <>{t('None')}</>;
}
function SourceFields({ source, t }: { source: FileSource; t: T }) {
  return <><dt>{t('Source path')}</dt><dd><code>{source.path}</code></dd><dt>{t('Recorded file state')}</dt><dd>{t(source.exists ? 'File exists' : 'File missing')}</dd><dt>{t('Source file hash (SHA-256)')}</dt><dd><code>{source.sourceDigest ?? t(source.exists ? 'Not recorded' : 'File missing')}</code></dd><dt>{t('Source identity')}</dt><dd><code>{source.sourceIdentity}</code></dd>{source.mode !== undefined && <><dt>{t('File mode')}</dt><dd><code>0o{source.mode.toString(8)}</code></dd></>}</>;
}
function ArtifactContent({ artifact, t }: { artifact: Artifact; t: T }) {
  return <div className="artifact-content"><dl className="io-metadata"><dt>{t('Artifact ID')}</dt><dd><code>{artifact.id}</code></dd><dt>{t('Output ID')}</dt><dd><code>{artifact.outputId ?? t('Not recorded')}</code></dd><dt>{t('Producer run')}</dt><dd><code>{artifact.runId}</code></dd><dt>{t(artifact.outputId || artifact.source ? 'Saved content hash (SHA-256)' : 'Recorded artifact hash')}</dt><dd><code>{artifact.digest}</code></dd>{artifact.source && <SourceFields source={artifact.source} t={t} />}</dl>{artifact.redacted && <p className="warning compact">{t('Saved content is redacted; it does not reproduce the original bytes.')}</p>}<Output content={artifact.content} truncated={artifact.truncated} t={t} /></div>;
}
function ArtifactRow({ artifact, t }: { artifact: Artifact; t: T }) {
  return <details className="artifact" data-artifact-id={artifact.id}><summary><FileCode2 /><span>{artifact.name}{artifact.outputId && <small> · {artifact.outputId}</small>}</span><small>{t(({ diff: 'Diff', file: 'File', text: 'Text summary' } as const)[artifact.kind])}{artifact.redacted && ` · ${t('Redacted')}`}{artifact.truncated && ` · ${t('Truncated')}`}</small></summary><ArtifactContent artifact={artifact} t={t} /></details>;
}
...
export default function App() {
  ...
  return <div className={`app ${navCollapsed ? 'nav-collapsed' : ''}`}>
    ...
    {impact && <Modal title={t('Review impact')} t={t} error={error} onClose={dismissRevision}><p>{t(impact.preview.reason as TextKey)}</p><p className="muted">{t('Generating an impact preview safely pauses the task. Dismissing it leaves the task paused; use Resume to continue unchanged.')}</p><p className="muted">{impact.task.title} · {t('Task revision')} {impact.preview.expectedRevision}</p>{impact.preview.checks !== undefined && <CheckComparison before={impact.task.checks} after={impact.preview.checks} t={t} manualAcceptance={impact.task.interaction !== 'conversation'} />}<ImpactList label={t('Affected steps')} ids={impact.preview.affected} reasons={impact.preview.nodeReasons} plan={impact.plan} t={t} /><ImpactList label={t('Retained steps')} ids={impact.preview.retained} reasons={impact.preview.nodeReasons} plan={impact.plan} t={t} /><p className="muted">{t('The workspace will keep its current files. Historical attempts remain available.')}</p>{impactStale && <p className="warning" role="alert">{t('This task or plan has changed. Close this preview and generate a new one.')}</p>}<div className="dialog-actions"><button className="button" onClick={dismissRevision}>{t('Dismiss')}</button><button className="button primary" disabled={!!busy || impactStale} onClick={() => void applyImpact()}>{t('Apply and continue')}</button></div></Modal>}
...
function ImpactList({ label, ids, reasons, plan, t }: { label: string; ids: string[]; reasons?: ImpactPreview['nodeReasons']; plan?: PlanRevision; t: T }) {
  return <div className="impact-list"><h3>{label}</h3>{ids.length ? <ul>{ids.map(id => { const reason = reasons?.find(item => item.nodeId === id)?.reason; return <li key={id}>{plan?.nodes.find(node => node.id === id)?.title ?? id}{reason && <p className="muted">{t(reason as TextKey)}</p>}</li>; })}</ul> : <p className="muted">{t('None')}</p>}</div>;
}
...
function deliveryLabel(binding: InputBinding): TextKey {
  if (binding.source?.exists === false) return 'Known missing file';
  if (!binding.content.length) return 'Empty saved content';
  if (!binding.deliveredRanges.length) return 'No delivery recorded';
  const ranges = [...binding.deliveredRanges].sort((a, b) => a.start - b.start);
  if (ranges.some(range => !Number.isInteger(range.start) || !Number.isInteger(range.end) || range.start < 0 || range.end < range.start || range.end > binding.content.length)) return 'Invalid delivery record';
  let end = 0;
  for (const range of ranges) { if (range.start > end) break; end = Math.max(end, range.end); }
  return end === binding.content.length ? (binding.redacted ? 'All saved redacted content delivered' : 'All saved content delivered') : 'Partly delivered';
}
function RunInputs({ run, t }: { run: TaskSnapshot['runs'][number]; t: T }) {
  return <section className="run-inputs"><h4>{t('Inputs for this run')}</h4><p className="input-coverage">{t('Input coverage')}: {t(run.inputCoverage === 'declared' ? 'Declared inputs tracked' : 'Unknown')}</p>{run.inputCoverage !== 'declared' && <p className="warning compact">{t('Input coverage is unknown; additional reads may not be recorded.')}</p>}
    {run.inputBindings ? run.inputBindings.length ? run.inputBindings.map((binding, index) => <details className="input-binding" key={index} data-input-index={index}><summary><IODeclaration value={binding.ref} t={t} /><small>{t(deliveryLabel(binding))}</small>{binding.redacted && <small>{t('Redacted')}</small>}</summary><dl className="io-metadata"><dt>{t('Saved content hash (SHA-256)')}</dt><dd><code>{binding.digest}</code></dd><dt>{t('Saved content length')}</dt><dd>{binding.content.length} {t('UTF-16 code units')}</dd><dt>{t('Delivered ranges (start included, end excluded)')}</dt><dd>{binding.deliveredRanges.length ? binding.deliveredRanges.map((range, rangeIndex) => <code key={rangeIndex}>[{range.start}, {range.end}){rangeIndex < binding.deliveredRanges.length - 1 ? ', ' : ''}</code>) : t('None')}<small className="range-unit">{t('Offsets refer to saved text in UTF-16 code units.')}</small><small className="range-unit">{t('Ranges record content supplied to the runner, not provider receipt or model comprehension.')}</small></dd>{binding.ref.kind === 'artifact' && <><dt>{t('Artifact ID')}</dt><dd><code>{binding.artifactId ?? t('Not recorded')}</code></dd><dt>{t('Producer run')}</dt><dd><code>{binding.producerRunId ?? t('Not recorded')}</code></dd></>}{binding.source ? <SourceFields source={binding.source} t={t} /> : binding.ref.kind === 'file' && <><dt>{t('Source')}</dt><dd>{t('Source information was not recorded.')}</dd></>}</dl>{binding.redacted && <p className="warning compact">{t('Saved content is redacted; delivery ranges cover only the saved text, not the original bytes.')}</p>}<details className="saved-input-content"><summary>{t('Saved input content')}</summary><Output content={binding.content} t={t} /></details></details>) : <Empty>{t('No fixed input bindings recorded')}</Empty> : <Empty>{t('Fixed inputs were not recorded for this run.')}</Empty>}
    <details className="file-reads"><summary>{t('Actual file reads')}{run.reads && ` · ${run.reads.length}`}</summary>{run.reads ? run.reads.length ? run.reads.map((read, index) => <dl className="io-metadata" key={index}><dt>{t('Source path')}</dt><dd><code>{read.path}</code></dd><dt>{t('Source file hash (SHA-256)')}</dt><dd><code>{read.sourceDigest}</code></dd><dt>{t('Read coverage')}</dt><dd>{t(read.complete ? 'Read in full' : 'Partial read')}</dd></dl>) : <Empty>{t('No file reads recorded')}</Empty> : <Empty>{t('File reads were not recorded for this run.')}</Empty>}</details>
  </section>;
}
function RunRecord({ snapshot, run, current, locale, t }: { snapshot: TaskSnapshot; run: TaskSnapshot['runs'][number]; current: boolean; locale: Locale; t: T }) {
  ...
  const reused = current ? snapshot.nodes.find(node => node.runId === run.id)?.reused : undefined;
  return <details className="receipt run-record" open={current} data-run-id={run.id}><summary><span>{t('Attempt')} {run.attempt}</span>{current ? <Status value={run.status} t={t} /> : <span className="muted">{t(run.status)}</span>}<small>{t(current ? 'Current' : 'Historical')}</small></summary>
    <div className="run-content"><div className="record-binding"><span>{t('Task revision')} {run.taskRevision} · {t('Plan ID')} {run.planId ?? t('Not recorded')}</span><code>{run.id}</code><time dateTime={run.startedAt}>{dateText(run.startedAt, locale)}</time>{run.endedAt && <span>{t('Ended')} · <time dateTime={run.endedAt}>{dateText(run.endedAt, locale)}</time></span>}</div>
      ...
      {!current && recordedNode && <details className="run-declarations"><summary>{t('Declarations for this run')}</summary><dl className="io-metadata"><dt>{t('Declared inputs')}</dt><dd><IODeclarations values={recordedNode.inputs} t={t} /></dd><dt>{t('Expected outputs')}</dt><dd><IODeclarations values={recordedNode.outputs} t={t} /></dd></dl></details>}
      {reused && <p className="muted" data-testid="research-reused">{t('Research retained')} · <time dateTime={reused.checkedAt}>{dateText(reused.checkedAt, locale)}</time><br />{t('Inputs and context were checked before retry. This is the original run and its outputs.')}</p>}
...
function PlanTracker({ snapshot, locale, t, onInspect }: { snapshot: TaskSnapshot; locale: Locale; t: T; onInspect: (id: string) => void }) {
  ...
  return <section className={`plan-tracker ${finished ? 'has-finished' : ''}`} aria-label={t('Task steps')}><div className="section-heading"><h3>{t('Task steps')}</h3><span>{verified + finished} / {plan.nodes.length} {t(conversation || finished ? 'Finished steps' : 'verified')}{(conversation || finished > 0) && ` · ${verified} ${t('verified')}`}</span></div><progress aria-label={t(conversation || finished ? 'Finished steps' : 'Verified steps')} max={Math.max(1, plan.nodes.length)} value={verified + finished} /><p className="tracker-caption">{t('Current plan')} · v{plan.revision} · {t('Recorded progress and outputs')}</p><ol className="step-list">{plan.nodes.map((node, index) => {
    ...
    return <li key={`${plan.id}:${node.id}`}><details className={`tracker-step tracker-${state?.status ?? 'queued'}`} data-node-id={node.id}><summary><span className="step-number">{state?.status === 'verified' ? <Check /> : index + 1}</span><strong>{node.title}</strong><span className="step-progress">{t(status)}</span></summary><div className="tracker-detail detail-content"><dl><dt>{t('Goal')}</dt><dd>{node.goal}</dd><dt>{t('Declared inputs')}</dt><dd><IODeclarations values={node.inputs} t={t} /></dd><dt>{t('Expected outputs')}</dt><dd><IODeclarations values={node.outputs} t={t} /></dd><dt>{t('Depends on')}</dt><dd>{node.dependsOn.map(id => plan.nodes.find(item => item.id === id)?.title ?? id).join('\n') || t('None')}</dd></dl>{state?.reason && <p className="warning">{t(state.reason as TextKey)}</p>}{state?.status === 'stale' && <p className="muted">{t('stale')}</p>}<StepEvidence snapshot={snapshot} node={node} plan={plan} locale={locale} t={t} /><button className="text-button" onClick={() => onInspect(node.id)}>{t('Open in planning')}</button></div></details></li>;
  })}</ol></section>;
}
    ...
    {prefs.panelView === 'process' ? <div className="process-content">
      ...
      {selectedNode && <section className="node-detail"><div className="detail-heading"><h3>{selectedNode.title}</h3>{nodeState && <Status value={nodeState.status} t={t} />}</div><small className="muted">{selectedNode.id}{nodeState && ` · ${t('Attempt')} ${nodeState.attempt}`}</small>
        ...
        <div className="detail-content">
          {prefs.detailTab === 'overview' && <><dl><dt>{t('Goal')}</dt><dd>{selectedNode.goal}</dd><dt>{t('Declared inputs')}</dt><dd><IODeclarations values={selectedNode.inputs} t={t} /></dd><dt>{t('Depends on')}</dt><dd>{selectedNode.dependsOn.map(id => source?.nodes.find(node => node.id === id)?.title ?? id).join('\n') || t('None')}</dd><dt>{t('Expected outputs')}</dt><dd><IODeclarations values={selectedNode.outputs} t={t} /></dd><dt>{t('Checks')}</dt><dd>{selectedNode.checkIds.map(id => definitions?.find(check => check.id === id)?.label ?? `${id} · ${t('Not recorded')}`).join('\n') || t('None')}</dd></dl>{nodeState?.reason && <p className="warning">{t(nodeState.reason as TextKey)}</p>}<StepEvidence snapshot={snapshot} node={selectedNode} plan={plan} locale={locale} t={t} />{!historical && plan && <button className="button retry-button" disabled={busy} onClick={() => void onRetry(selectedNode.id)}><Play />{t('Retry step')}</button>}</>}
