...
function usageText(runs: TaskSnapshot['runs'], field: 'input' | 'output', locale: Locale, t: T) {
  const reported = runs.filter(run => run.usage && Number.isFinite(run.usage[field]));
  if (!reported.length) return t('Unavailable');
  return reported.reduce((sum, run) => sum + run.usage![field], 0).toLocaleString(resolveLocale(locale)) + (reported.length < runs.length || reported.some(run=>run.usage?.partial) ? ` · ${t('Partial usage')}` : '');
}
    ...
    {prefs.panelView === 'process' ? <div className="process-content">
      ...
      {selectedNode && <section className="node-detail"><div className="detail-heading"><h3>{selectedNode.title}</h3>{nodeState && <Status value={nodeState.status} t={t} />}</div><small className="muted">{selectedNode.id}{nodeState && ` · ${t('Attempt')} ${nodeState.attempt}`}</small>
        ...
        <div className="detail-content">
          ...
          {prefs.detailTab === 'checks' && <>{checks.map(check => {
            const current = !historical && !!plan && snapshot.task.activePlanId === plan.id && check.planId === plan.id && check.taskRevision === snapshot.task.revision && nodeState?.runId === check.runId && !['stale', 'unknown', 'failed'].includes(nodeState?.status ?? '') && !!check.batchId && check.scope === 'node' && !!check.checksDigest && !!check.inputDigest;
            return <details key={check.id} className="receipt"><summary>{current ? <Status value={check.result} t={t} /> : <span className="muted">{t(check.result)}</span>}<span>{snapshot.task.checks.find(item => item.id === check.conditionId)?.label ?? check.conditionId}</span><small>{t(current ? 'Current' : 'Historical')}</small></summary><Output content={check.output} t={t} /><dl><dt>{t('Batch')}</dt><dd>{check.batchId ?? t('Not recorded')}</dd><dt>{t('Scope')}</dt><dd>{check.scope ? t(check.scope === 'node' ? 'Step' : 'Final acceptance') : t('Not recorded')}</dd><dt>{t('Task revision')}</dt><dd>{check.taskRevision ?? t('Not recorded')}</dd><dt>{t('Plan ID')}</dt><dd>{check.planId ?? t('Not recorded')}</dd><dt>{t('Run')}</dt><dd>{check.runId}</dd><dt>{t('Input digest')}</dt><dd>{check.inputDigest}</dd><dt>{t('Checks digest')}</dt><dd>{check.checksDigest ?? t('Not recorded')}</dd><dt>{t('Recorded')}</dt><dd>{dateText(check.checkedAt, locale)}</dd></dl></details>;
          })}{!checks.length && <Empty>{t('No checks yet')}</Empty>}</>}
