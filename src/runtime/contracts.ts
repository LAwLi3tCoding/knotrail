import type { CheckSpec, FileRead, ModelConfig, PlanDraft, PlanNode, PlanRevision, WaitRequest } from '../shared/contracts.js';
export interface ToolCall { toolCallId: string; name: 'read_file' | 'read_input' | 'list_files' | 'search_files' | 'write_file' | 'edit_file' | 'run_command'; args: Record<string, unknown> }
export interface ToolResult { text: string; isError?: boolean; source?: FileRead }
export type RunControl = { kind: 'update_plan'; draft: PlanDraft; submit: boolean } | { kind: 'complete'; summary: string } | { kind: 'decision'; question: string; options: string[] } | ({ kind: 'wait' } & WaitRequest) | { kind: 'blocked'; reason: string };
