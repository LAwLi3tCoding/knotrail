...
process.on('message', (message: { kind: string; request: RunnerRequest; id: number; value: ToolResult }) => {
  if (message.kind === 'reply') { pending.get(message.id)?.(message.value); pending.delete(message.id); }
...
async function main(request: RunnerRequest): Promise<RunnerResult> {
  ...
  runtime.registerProvider('knotrail', {
  ...
  await runtime.setRuntimeApiKey('knotrail', request.model.apiKey || 'local-no-key');
  ...
  let summary = '';
  tools.push({ name: 'outcome', label: 'Report outcome', description: 'End the current Run with a complete report, user decision, external wait, or obstruction.', parameters: Type.Object({ kind: Type.Union([Type.Literal('complete'), Type.Literal('decision'), Type.Literal('wait'), Type.Literal('blocked')]), summary: Type.Optional(Type.String()), question: Type.Optional(Type.String()), options: Type.Optional(Type.Array(Type.String())), reason: Type.Optional(Type.String()), minutes: Type.Optional(Type.Number({ minimum: 1, maximum: 10080 })) }), executionMode: 'sequential', execute: async (_id, args) => { const control = args as RunControl;
    if ((control.kind === 'complete' && !control.summary) || (control.kind === 'decision' && (!control.question || !Array.isArray(control.options))) || (control.kind === 'wait' && (!control.reason || !Number.isFinite(control.minutes) || control.minutes < 1)) || (control.kind === 'blocked' && !control.reason)) return text({ text: 'Outcome fields do not match the selected kind', isError: true });
  ...
  try {
    try { await session.prompt(JSON.stringify({ objective: request.objective, purpose: request.purpose, checks: request.checks, plan: request.plan, node: request.node, context: request.context }), { expandPromptTemplates: false }); } catch (error) { if (!closed && !aborted) throw error; }
