...
export interface RetryCheckpoint { taskRevision: number; planId: string; afterRunCount: number; workspaceDigest: string; sourcesDigest: string }
...
export interface ImpactPreview { reuseDigest?: string; nodeReasons?: { nodeId: string; reason: string }[];  id: string; taskId: string; expectedRevision: number; expectedPlanId?: string; workspaceDigest: string; inputSourcesDigest?: string; objective?: string; checks?: CheckSpec[]; nodeId?: string; affected: string[]; retained: string[]; reason: string }
