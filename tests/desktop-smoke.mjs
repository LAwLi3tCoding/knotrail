...
const live = process.env.KNOTRAIL_AUTH_SOURCE === 'codex-login';
const modelId = live ? process.env.KNOTRAIL_MODEL_ID || 'gpt-6-astra' : 'desktop-fixture';
...
const objective = 'Replace target.txt with the content of expected.txt, preserving the final newline. First inspect and submit exactly one edit plan node including required check ID compare. Then read target.txt, edit with write_file and expectedContent, and report complete. Do not change expected.txt.';
const draft = {
...
try {
  ...
  await command({ type: 'settings.save', patch: { locale: 'en', model: { authSource: live ? 'codex-login' : 'api-key', baseUrl, modelId, thinking: live ? 'high' : 'off', contextWindow: 32000, maxTokens: 1024 } } });
  await command({ type: 'model.check' });
  const project = await command({ type: 'project.add', path: source });
  snapshot = await command({ type: 'task.create', requestId: 'desktop-fixture-create', projectId: project.id, objective, checks: [{ id: 'compare', label: 'Compare the result', command: ['/usr/bin/cmp', 'target.txt', 'expected.txt'], protectedPaths: ['expected.txt'] }], executionPolicy: 'reviewBeforeExecute', mode: 'once', maxTurns: live ? 16 : 10, maxRunMs: live ? 180000 : 30000 });
  ...
  const until = async status => {
    ...
    }, { timeout: live ? 210_000 : 45_000, intervals: [100, 200, 500] }).toBe(status);
  };
  await until('ready');
  if (live) console.log(JSON.stringify({ event: 'desktop.ready', modelId, nodes: snapshot.plan.nodes.length }));
  ...
  await page.locator('.task-row', { hasText: snapshot.task.title }).click();
